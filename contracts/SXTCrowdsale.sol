...
contract MintableToken is StandardToken, Ownable {
    string public constant name = "Spaceship Token";
...
contract SXTCrowdsale is Ownable, Crowdsale, MintableToken {
    using SafeMath for uint256;
    ...
    uint256 public reservFund = 1 * (10**6) * (10 ** uint256(decimals));
    uint256 public tokenCapReached = 4 * (10**6) * (10 ** uint256(decimals));
    ...
    function getTotalAmountOfTokens(uint256 _weiAmount) internal constant returns (uint256 amountOfTokens) {
        ...
        if (currentDate >= startTime && currentDate < endTimeStageOne) {
            return currentTokenRate = _weiAmount.mul(13000);
        } else if (currentDate >= endTimeStageOne) {
            return currentTokenRate = _weiAmount.mul(10000);
        } else {
